...
def gameshow(cat, score, read, index): #defines the gameshow and has the parimeters the game must run through
    ...
    def any_line(catergory1):
        ...
        return [pick_line[index]], pick_line[index + 1]
...
for x in range(len(read)-1):
    if usernames+"\n" == read[x]: #try to find teh username in the file
        score = int(read[x + 1]) #will pick the line below the username and define it as the score
        ...
        f.write("\n" + usernames) #does the same thing as the line above
        f.close()
        ...
        if category == "category 1" or category == "1": #determinnes what category the user selected
            gameshow("catergory 1", 0, read, x+1) #calls on the gameshow function and runs it through the parameters
        elif category == "category 2" or category == "2": #determinnes what category the user selected
            gameshow("catergory 2", 0, read, x+1) #calls on the gameshow function and runs it through the parameters
        elif category == "category 3" or category == "3": #determinnes what category the user selected
            gameshow("catergory 3", 0, read, x+1) #calls on the gameshow function and runs it through the parameters
        elif category == "category 4" or category == "4": #determinnes what category the user selected
            gameshow("catergory 4", 0, read, x+1) #calls on the gameshow function and runs it through the parameters
